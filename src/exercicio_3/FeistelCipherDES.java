...
public class FeistelCipherDES {
    ...
    // Função para pegar uma SBOX do DES
    private static int[][][] sBoxes = {
        ...
        public static int[][] getSBox(int number) {
            if (number >= 1 && number <= sBoxes.length) {
                return sBoxes[number - 1];
            } else {
                System.out.println("S-box não encontrada");
                return null;
            }
        }
